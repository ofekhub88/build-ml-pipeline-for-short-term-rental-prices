...
def go(args):
    ...
    X_processed_features = X_val[processed_features]
    objects_cols = X_processed_features.select_dtypes(include=['object']).columns.tolist()
    X_processed_features[objects_cols] = X_processed_features[objects_cols].astype(str)

    signature = mlflow.models.infer_signature(X_processed_features, y_pred)
